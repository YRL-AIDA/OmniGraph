...
def main():
    ...
    ckpts = os.listdir(model_args.model_name_or_path)
    folders_with_ckpt_prefix = [
    item for item in ckpts
    if os.path.isdir(os.path.join(model_args.model_name_or_path, item)) and item.startswith('checkpoint')
    ]
    eval_results = {}
    pred_results = {}
    for ckpt in folders_with_ckpt_prefix:
        check_dir = os.path.join(model_args.model_name_or_path, ckpt)
        checkpoint_num = ckpt.split('-')[-1].strip()
