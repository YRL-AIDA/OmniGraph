        ...
        if len(model_input['input_ids']) <= tokenizer.model_max_length:
            if padding == "max_length" and ignore_pad_token_for_loss:
                model_input['labels'] = [
                    ...
                    for label in tokenizer(example['answer'],padding=padding, truncation=False)['input_ids']
                    ]
                return model_input
        else:
...
class DataTrainingArguments:
    ...
    overwrite_output_dir: bool = field(
        default=False, metadata={"help": "Overwrite the output dir"}
    )
...
def main():
    ...
    if os.path.isdir(training_args.output_dir) and training_args.do_train and not data_args.overwrite_output_dir :
    ...
    if data_args.dataset_name is not None:
        ...
        dataset = load_from_disk(data_args.castom_data_local_dir)
        dataset = dataset.train_test_split(test_size=0.2, shuffle=True,seed=training_args.seed)
        dataset['train'],dataset['validation'] = dataset['train'].train_test_split(test_size=0.25, 
                                                                                   shuffle=True,
