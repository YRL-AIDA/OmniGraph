...
from add_utils import deserializ_tapex_linear_table,escape_special_characters,get_sql_and_table, get_sql_and_table_semantic_friendly, fix_sql_in_tapex_query,serialize_table_to_tapex_format
...
def replace_query_headers(query,headers_dict):
    if query != None :
        head  = {item[1][1]: item[0] for item in headers_dict.items()}
        return " ".join([head.get(word,word) for word in query.split(' ')])
    else:
        return None

def replace_table_headers(table,headers_dict):
    head  = {item[1][1]: item[0] for item in headers_dict.items()}
    table_new_headers = [head.get(column_name,column_name) for column_name in table.columns]
    table.columns = table_new_headers
    return table

def replace_answer_headers(answer,headers_dict):
    columns  = {item[1][1]: escape(str(item[0])) for item in headers_dict.items()}
    
    for head in columns:
        answer = re.sub(head,columns[head],answer)
        
    return answer
    
def get_tapex_execution_plan_semantic_friendly(question,tab_name='w'):
    sql,tbl,headers_dict = get_sql_and_table_semantic_friendly(question)
    answer = None
    sql = fix_sql_in_tapex_query(sql,tab_name=tab_name)
    if sql != None or "None":
        ...
        answer = get_query_execution_plan(tbl,sql,tab_name=tab_name)
        #print("PLANING",answer)
    return f'{replace_query_headers(sql,headers_dict)} {serialize_table_to_tapex_format(replace_table_headers(tbl,headers_dict))}', replace_answer_headers(answer[0][0],headers_dict) if answer != None else answer
