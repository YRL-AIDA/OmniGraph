...
from utils import get_sqlite_data,get_tapex_execution_plan_semantic_friendly
from datasets import concatenate_datasets,load_from_disk,Dataset
...
def map_function_for_question_change(example):
    ...
    query,plan = get_tapex_execution_plan_semantic_friendly(example['question'], tab_name=tab_name)
...
while True:
    ...
    dataset.save_to_disk(f'./converved_to_plan_tapex_data_semantic_friendly_{chank_id}')
    chank_id+=1
# In[10]:
dataset = Dataset.from_dict({
        'question': [],
...
for chank in range(chank_id):
    dataset_path = f'./converved_to_plan_tapex_data_semantic_friendly_{chank}'
    data = load_from_disk(dataset_path)
    dataset = concatenate_datasets([dataset,data])
    del data
    shutil.rmtree(f'./converved_to_plan_tapex_data_semantic_friendly_{chank}')
print("DATA TRANSFORMED. START SAVING")
dataset.save_to_disk(f'./converved_to_plan_tapex_data_semantic_friendly_full')
