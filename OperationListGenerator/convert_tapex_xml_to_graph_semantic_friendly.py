...
def map_function_for_question_change(example):
    ...
    try:
        ...
        print(e)
        print(example['question'])
        print(type(example['xml_plan']))
        print(example['xml_plan'])
    return example
dataset = load_from_disk('./converved_to_plan_tapex_data_semantic_friendly_full')   
dataset = dataset.map(map_function_for_question_change,num_proc=17)
print("DATA TRANSFORMED. START SAVING")
dataset.save_to_disk(f'./converved_to_graph_plan_tapex_data_semantic_friendly_full')